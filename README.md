...
The **DAT lists** are what turn raw DAT paths into names — every zone in the Zone
List, NPC and monster models, music, sound effects. They are generated by `xi mv
update` in [xi-tools](https://github.com/vekien/xi-tools), which publishes them
with `mv/lists/manifest.json`: a sha256 and byte count per file. This is the same
set the [model viewer](https://github.com/vekien/xi-model-viewer) reads, so a
zone named in one is named in the other.

`ui/public/lists/` is a copy of that folder, manifest included, baked into the
`.exe` — so the app works with no network at all. On launch it fetches xi-tools'
manifest and downloads only the files whose contents no longer match the copy it
holds, into `%LOCALAPPDATA%\XiZoneEditor\lists\`. A zone renamed upstream then
shows up without waiting for a new release.

The comparison is content-addressed, not versioned: there is no number to bump on
either side, a list reverted in xi-tools goes back to matching on its own, and a
download that gets corrupted is simply re-fetched next boot. Each file is checked
against its published sha256 and byte count before it replaces anything, and
lands under a `.part` name until it passes — a failed or interrupted download
leaves the previous copy alone.

Being offline is a normal outcome: the check gets ten seconds and then gives up
quietly, on the lists already loaded. **Settings → Setup → DAT Lists** shows which
copy of each list is in effect and has the one button that goes to the network.

Releases bake whatever is on xi-tools' `main` at build time. To bake a local
xi-tools checkout instead — to try a list before pushing it:
...
Setup only happens once, so the flow a new user meets is the one you stop seeing
the day you finish setting up. `Reset.bat` puts it back:

```bat
Reset.bat
```

It renames every piece of stored state aside as `<name>.bak-<timestamp>` — saved
settings, the downloaded xi-tools and Python, and the `.env` holding your game
paths and database login — so the next launch is a genuine first run. Nothing is
deleted, and your projects and zone edits are left alone unless you pass `/all`.

| | |
|---|---|
| `Reset.bat` | Park the app's state, keeping your projects |
| `Reset.bat /all` | Also park your projects and zone edits |
| `Reset.bat /restore` | Put the most recent backup of each back |

Close the editor first — it writes its settings back out on exit. Note this gives
you a clean first run of a **dev** build; to see what someone downloading a
release actually gets, run the built `.exe`.
...
| `ui/ffxi/lists.js` | One door for `lists/*.json` — downloaded copy first, baked copy otherwise |
| `ui/public/exports/assets/` | Bundled Asset Browser data |
| `ui/public/lists/` | Baked DAT lists (zones, NPCs, models, music, SFX). Generated by `xi mv update` in xi-tools — not by anything in this repo. `manifest.json` is xi-tools' index of the set by sha256, copied here with them; it is how the app refreshes these at runtime (see [Updating the lists](#updating-the-lists)) |
| `src-tauri/` | Tauri shell |
| `src-tauri/src/lists.rs` | Fetches xi-tools' manifest and replaces any list whose contents have moved |
| `Start.bat` / `Build.bat` / `Reset.bat` | Dev / release / back to a first-run state |

| Runtime | Location |
|---------|----------|
| Downloaded tools, embed Python, bridge log | `%LOCALAPPDATA%\XiZoneEditor\` |
| Updated DAT lists | `%LOCALAPPDATA%\XiZoneEditor\lists\` |
| Saved settings (projects list, pins, last zone) | `%LOCALAPPDATA%\uk.co.viion.xizoneeditor\EBWebView\` |
